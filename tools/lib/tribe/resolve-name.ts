...
const KNOWN_FLAVORS = ["codex", "claude", "kimi", "gemini", "pi"] as const
...
export function resolveName(ctx: ResolveNameContext): string {
  const { db, p, adopted, claudeSessionName, claudeSessionId, role, isActive, projectId, takenNames } = ctx
  ...
  //    Today this is rarely set; when it is, honour it.
  if (claudeSessionName) return claudeSessionName

  // 3. Identity-token adoption — same Claude Code session resuming.
  if (adopted?.name) return adopted.name

  // 4. Prior row matched by claude_session_id.
  ...
  // 5. Project-and-role adoption — same project, same role, prior session.
  const projectAdopted = adoptByProjectAndRole(db, projectId, role, isActive)
  if (projectAdopted) return projectAdopted.name

  // 6. Chief role keeps the special name.
  if (role === "chief") return "chief"

  // 7. No signal — default to claude-flavor auto-number. This is the
  //    common path for fresh Claude Code MCP clients (no TRIBE_NAME env,
  //    no claudeSessionName, no prior). Before this rule, the fallback
  //    was `projectName` (e.g. "km") which was both ambiguous (every
  //    session looked alike) and collided with peer sessions.
  return assignFlavorName("claude", takenNames)
}
